...
/**
 * Launch Chromium with a fail-safe that installs the browser
 * at runtime if it's missing (useful on platforms like Railway).
...
async function launchBrowserWithFallback() {
  ...
  try {
    ...
    console.error(`Browser missing, installing now: ${e.message || e}`);

    try {
      // Respect PLAYWRIGHT_BROWSERS_PATH / PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD
      // configured in the Docker image / environment.
