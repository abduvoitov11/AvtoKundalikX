...
// Start bot with webhook cleared first to avoid 409 conflicts.
async function startBot() {
  try {
    console.log('Deleting Telegram webhook (drop_pending_updates=true) before launch...');
    await bot.telegram.deleteWebhook({ drop_pending_updates: true });
  } catch (e) {
    console.error('Failed to delete webhook before launch:', e.message || e);
  }

  try {
    await bot.launch();
    console.log('eMaktab Telegram bot running. Cron: 07:45 Tashkent.');
  } catch (e) {
    console.error('Bot launch failed:', e.message || e);
    // Handle 409 Conflict (bot already running or webhook still active) gracefully.
    if (e.response && e.response.error_code === 409) {
      console.error(
        'Telegram 409 Conflict: bot appears to be running elsewhere or with an active webhook. Exiting gracefully.'
      );
    }
    process.exit(1);
  }
